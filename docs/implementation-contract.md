...
A completed contract version is immutable except for which version is active. Generating a replacement never mutates the old version. A version is current only while both its document source hash and selected Research Map signature still match current inputs. Activating a stale history version changes the active version; it does not make that version current. Decisions are accepted only on the version that is both active and the unique lineage tip. An active ancestor and an inactive tip are both visibly read-only; activate the latest tip before deciding. This prevents ancestor/descendant revision forks and keeps the post-save workspace tied to the decided version. Generation always continues from the unique lineage tip even if an ancestor is currently active, and refuses an already-forked history.

Historical evidence links load the exact retained Reader text snapshot and exact Research Map version recorded by that Contract, rather than substituting the document's current lineage. Every generated **View original page** URL includes the retained source SHA-256, and the page endpoint hashes the source file on disk again before serving it. Glyph does not retain versioned source files: when that live hash differs from the retained text snapshot, Reader disables the link and labels the original page unavailable instead of opening a misleading current page. A previously copied page URL likewise returns `409` after the source file changes, even before a Library refresh.
...
Human resolutions are append-only revisions with a request ID, contract version, stable item key, and item signature. The signature includes that `item_key` plus normalized type, draft value, origin, rationale, and evidence hashes. A resolution carries forward only within the same document when both the item key and signature are byte-for-byte identical. A later decision on an identical regenerated item continues the document-lineage revision number and supersedes the effective carried decision. Any change to either identity component requires new review. A stale signature or conflicting request returns `409`; reload the contract before deciding again. After a save, the UI hides readiness until it reloads the backend's authoritative audit; a reload failure is shown as **Readiness unverified**, never as implementation-ready.

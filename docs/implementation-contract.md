...
The provider creates an immutable draft. A later `corrected` or `decided` resolution changes only the effective value and effective origin. The draft, evidence, and earlier resolutions remain available for audit.
...
- `not_applicable` requires a reason and no replacement value;
- a missing blocking value stays `blocked` unless a valid human decision resolves it;
- a missing transaction-cost item may be marked not applicable only for an explicitly reasoned gross replication.
...
A completed contract version is immutable except for which version is active. Generating a replacement never mutates the old version. A version is current only while both its document source hash and selected Research Map signature still match current inputs. Activating a stale history version changes the active version; it does not make that version current.

Deterministic diffs classify each stable `item_key` as `unchanged`, `value_changed`, `origin_changed`, `evidence_changed`, `added`, or `removed`.

Human resolutions are append-only revisions with a request ID, contract version, and item signature. A resolution carries forward only within the same document when the new item signature is byte-for-byte identical. Any change to normalized type, draft value, origin, rationale, or evidence hashes requires new review. A stale signature or conflicting request returns `409`; reload the contract before deciding again.
...
curl -sS http://127.0.0.1:8000/api/documents/DOCUMENT_ID/implementation-contract/versions

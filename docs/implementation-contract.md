...
A completed contract version is immutable except for which version is active. Generating a replacement never mutates the old version. A version is current only while both its document source hash and selected Research Map signature still match current inputs. Activating a stale history version changes the active version; it does not make that version current. Historical evidence links load the exact retained Reader text snapshot and exact Research Map version recorded by that Contract, rather than substituting the document's current lineage. Glyph does not retain versioned source files: when the current file hash differs from the retained text snapshot, Reader disables **View original page** and labels the original page unavailable instead of opening a misleading current page.
